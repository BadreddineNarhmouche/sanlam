...
const initKeycloak = (
  ...
  onAuthenticatedFailedCallback: (args?: any) => any
): any => {
  keycloak
    .init({
      useNonce: true,
...
const isAuthenticated = (): any => keycloak.authenticated;
...
const isAuthorized = (internalRoleCodes) => {
  ...
  if (
    LocalStorageHelper.contains(
      LocalStorageKeyConstants.internalUserInternalRoles
    )
  ) {
    let data = LocalStorageHelper.get(
      LocalStorageKeyConstants.internalUserInternalRoles
    );

    if (GeneralHelper.isArrayNotNullOrEmpty(data)) {
      const containsList = (mainList, subList) =>
        subList.some((item) => mainList.includes(item));
...
const getCurrentInternalUser = (): Promise<any> => {
  return new Promise((resolve, reject) => {
    if (isAuthenticated()) {
      if (LocalStorageHelper.contains(LocalStorageKeyConstants.internalUser)) {
        const internalUser = LocalStorageHelper.get(
        ...
        fetch(
          `${process.env.REACT_APP_API_BASE_PATH}/InternalUsers/GetByElectronicAddress` +
