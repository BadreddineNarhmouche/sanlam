...
import { DocumentTypeCodeConstants } from "@reinsurance/helpers/lib/helpers/ConstantsHelper";
import { CircularProgress } from "@mui/material";

export const ReinsuranceValidation = ({
  status,
  readOnly,
  titleDoing,
  titleDone,
  ...
  const {
    responseData: deliverySlipValidateReinsurance,
    isLoading: isLoadingDeliverySlipValidateReinsurance,
    error: errorDeliverySlipValidateReinsurance,
  } = useSelector((state: any) => state.deliverySlipValidateReinsurance);
  ...
  const {
    responseData: deliverySlipGetDocumentByCriteria,
    isLoading: isLoadingDeliverySlipGetDocumentByCriteria,
    error: errorDeliverySlipGetDocumentByCriteria,
  } = useSelector((state: any) => state.deliverySlipGetDocumentByCriteria);

  const {
    responseData: deliverySlipDownloadDocumentByCriteria,
    isLoading: isLoadingDeliverySlipDownloadDocumentByCriteria,
    error: errorDeliverySlipDownloadDocumentByCriteria,
  } = useSelector((state: any) => state.deliverySlipDownloadDocumentByCriteria);

  const [selectedSettlementNoteFile, setSelectedSettlementNoteFile] =
    useState<File | null>(null);
  const [selectedDeliverySlipDetailFile, setSelectedDeliverySlipDetailFile] =
    useState<File | null>(null);
  ...
  const handleSubmit = () => {
    services.validateReinsuranceDeliverySlip &&
      services.validateReinsuranceDeliverySlip({
        externalTimelineHistoryId: currentTimelineId,
        externalNextTransitionId: nextTransitions[0]?.id,
        settlementNote: selectedSettlementNoteFile,
        deliverySlipDetail: selectedDeliverySlipDetailFile,
  ...
  const displayTransitionFile = (id: string) => {
    services.getDeliverySlipDocumentById &&
      services.getDeliverySlipDocumentById({ id });
  };
  ...
  useEffect(() => {
    if (deliverySlipGetDocumentById?.content) {
      ...
      setDialogOpen(true);
      services.clearGetDeliverySlipDocumentById &&
        services.clearGetDeliverySlipDocumentById();
  ...
  const onDownloadFile = (file: any, documentTypeCode: string) => {
    if (file) {
      ...
      services.downloadDeliverySlipDocumentByCriteria &&
        services.downloadDeliverySlipDocumentByCriteria({
          deliverySlipId: demand.id,
          documentTypeCode,
          bankAccountId: bankSelect?.id ?? "",
          equivalent: checked,
          refReinsurer: inputValue,
          currencyId: currencySelect?.id ?? "",
        });
    }
  };
  ...
  const onDisplayFile = (file: any, documentTypeCode?: string) => {
    if (file) {
      ...
      services.getDeliverySlipDocumentByCriteria &&
        services.getDeliverySlipDocumentByCriteria({
          deliverySlipId: demand.id,
          documentTypeCode,
          bankAccountId: bankSelect?.id ?? "",
  ...
  useEffect(() => {
    if (deliverySlipGetDocumentByCriteria?.content) {
      const url = window.URL.createObjectURL(
        new Blob([
          GeneralHelper.base64ToUint8Array(
            deliverySlipGetDocumentByCriteria?.content
          ),
        ])
      );
      setPDFViewer &&
        setPDFViewer((prevState) => ({
          ...prevState,
          file: url,
          fileName: `${deliverySlipGetDocumentByCriteria.name}`,
          open: true,
        }));
      setDialogOpen(true);
      services.clearGetDeliverySlipDocumentByCriteria &&
        services.clearGetDeliverySlipDocumentByCriteria();
      setDocumentIdToView(null);
      setChecked(false);
    }
    if (deliverySlipGetDocumentByCriteria?.isSuccess) {
      setDataOpen(false);
    }
  ...
  const [currencySelect, setCurrencySelect] = useState<any>();

  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setChecked(event.target.checked);
  };

  const handleChangeInputRef = (event: React.ChangeEvent<HTMLInputElement>) => {
    setInputValue(event.target.value);
  };

  const myCallFunction = () => {
    onDisplayFile(
      selectedSettlementNoteFile,
      DocumentTypeCodeConstants.settlementNote
    );
  };
          ...
          {readOnly ? (
            ...
            >
              <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                  <Typography variant="body2" color="base.greyMain" mb={2}>
                  ...
                  <Grid item container spacing={2}>
                    <Grid item xs={6}>
                      <FileListItem
                        ...
                        documentTypes=".pdf"
                        setFile={(file) => {
                          setSelectedSettlementNoteFile(file);
                        }}
                        handleDownloadFile={() =>
                          onDownloadFile(
                            selectedSettlementNoteFile,
                        ...
                        handleViewFile={() =>
                          selectedSettlementNoteFile
                            ? onDisplayFile(selectedSettlementNoteFile)
                            : setDataOpen((res) => !res)
                        }
                        handleUploadFile={() => {}}
                        displayFile
                        isLoading={
                          isLoadingDeliverySlipDownloadDocumentByCriteria &&
                          documentIdToDownload ===
                            DocumentTypeCodeConstants.settlementNote
                        }
                        isLoadingView={
                          isLoadingDeliverySlipGetDocumentByCriteria &&
                          documentIdToView ===
                            DocumentTypeCodeConstants.settlementNote
                        }
                        error={
                          errorDeliverySlipDownloadDocumentByCriteria &&
                          documentIdToDownload ===
                            DocumentTypeCodeConstants.settlementNote
                        }
                        errorView={
                          errorDeliverySlipGetDocumentByCriteria &&
                          documentIdToView ===
                            DocumentTypeCodeConstants.settlementNote
                        }
                      />
                    ...
                    <Grid item xs={6}>
                      <FileListItem
                        ...
                        documentTypes=".pdf"
                        setFile={(file) => {
                          setSelectedDeliverySlipDetailFile(file);
                        }}
                        handleDownloadFile={() =>
                          onDownloadFile(
                            selectedDeliverySlipDetailFile,
                            DocumentTypeCodeConstants.deliverySlipDetail
                          )
                        }
                        handleViewFile={() =>
                          onDisplayFile(
                      ...
                      <Grid item xs={4}>
                        <Autocomplete
                          ...
                          renderInput={(params: any) => (
                            <TextField {...params} label="Compte bancaire" />
                          )}
                        />
                      </Grid>
                      <Grid item xs={4} mt={2}>
