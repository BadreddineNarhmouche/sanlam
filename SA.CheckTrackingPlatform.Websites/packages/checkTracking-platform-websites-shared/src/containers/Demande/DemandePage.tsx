...
export const DemandePage = ({
  ...
  const {
    responseData: quittancesOther,
    meta: quittancesMeta,
    isLoading: isLoadingQuittancesData,
    error: errorQuittancesData,
  } = useSelector((state: any) => state.quittancesOther);

  const { responseData: quittanceReinsurer } = useSelector(
    (state: any) => state.quittanceReinsurer
  );

  const { responseData: quittanceStatus } = useSelector(
    (state: any) => state.quittanceStatus
  );
  ...
  const [quittanceStatusData, setQuittanceStatusData] = useState<any[]>([]);

  const handleSubmit = (values: FilterCriteriaQuittances) => {
    const criteria = {
      ...values,
      workFlowStepCode: WORKFLOW_STEP_CODES.OTHER,
      policyReference: values.policyReference,
      reference: values.reference,
      quittanceStatusId:
        quittanceStatus.find((c: any) => c.id === values.quittanceStatusId)
          ?.code ?? "",
    ...
    services.getAllOtherQuittancesByCriteria &&
      services.getAllOtherQuittancesByCriteria(criteria);

    services.reinsurerByAll && services.reinsurerByAll();

    services.getQuittanceStatus &&
      services.getQuittanceStatus({ isHide: false });
  ...
  useEffect(() => {
    setQuittancesData(quittancesOther);
  }, [quittancesOther]);
  ...
  useEffect(() => {
    setQuittanceStatusData(quittanceStatus);
  }, [quittanceStatus]);
    ...
    services.getAllOtherQuittancesByCriteria &&
      services.getAllOtherQuittancesByCriteria({
        ...
        quittanceStatusId: filterValues?.quittanceStatusId
          ? quittanceStatus?.find(
              (c: any) => (c.id = filterValues?.quittanceStatusId)
            )?.code
          : "",
